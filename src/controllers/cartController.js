...
const addToCart = asyncHandler(async (req, res) => {
  ...
  if (qty <= 0) {
    throw new ApiError(400, "Quantity must be greater than 0", "INVALID_QUANTITY");
  }
  ...
  return res.status(201).json(
    new ApiResponse(201, { product:productId, quantity: qty }, "Product has been added to cart")
  );
});
...
const deleteFromCart = asyncHandler(async (req, res) => {
    ...
    return res
        .status(200)
        .json(new ApiResponse(200, productId, "Product has been removed from cart"));
});
...
const updateCartItemQuantity = asyncHandler(async (req, res) => {
    ...
    switch (action) {
        ...
        case "decrement":
            if (cartItem.quantity <= 1) {
                throw new ApiError(400, "Quantity cannot be less than 1", "MIN_QUANTITY_REACHED");
            }
            incrementValue = -1;
            break;
    ...
    return res
        .status(200)
        .json(new ApiResponse(200, { productId, quantity: cartItem.quantity + incrementValue }, `Product quantity ${action}ed successfully`));
});
